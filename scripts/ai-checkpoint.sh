...

{
  ...
  if [ "$onda_run_tests" -eq 1 ]; then
    # Site estático (HTML/CSS/JS vanilla, sem build nem suíte). Validação é
    # manual: servidor estático local, conferir scrollytelling, navegação
    # centralizada e o carrossel mobile de serviços.
    echo "| (sem suíte automatizada — site estático) | — | conferência manual no navegador |"
  else
